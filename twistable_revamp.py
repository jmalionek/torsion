...
import math
from sage.all import (RR,ZZ,matrix,DiGraph,ChainComplex,FreeGroup,Graph,vector, AbelianGroup,MatrixSpace)
import sage
import snappy
import d_domain
import torsion_poly
...
#---------------------------SETUP-----------------------
	...
	def _setup_vertex_orbits(self):
		...
		for index,vertex_dict in enumerate(self.D.vertex_list(True)):
			...
			if self.vertex_orbits[orbit_index]==None:
	...
	def _setup_edge_orbits(self):
		...
		for index,edge_dict in enumerate(self.D.edge_list()):
			...
			if self.edge_orbits[orbit_index]==None:
			...
			for face_index, face in enumerate(self.D.face_list()):
				...
				for index,vertex_index in enumerate(face['vertex_indices']):
					if self.vertices[vertex_index]!=None:
						...
						if self.vertices[glued_index] == None:
							#ORIGINALLY
		...
		while None in self.edges:
			for face_index, face in enumerate(self.D.face_list()):
				...
				for index, edge_index in enumerate(face['edge_indices']):
					if self.edges[edge_index]!=None:
						glued_index = face['edge_image_indices'][index]
						if self.edges[glued_index] == None:
							#ORIGINALLY
	...
	def _setup_faces(self):
		...
		for i, face_dict in enumerate(self.D.face_list()):
			...
			if i % 2 == 0:
				...
				face = Face(None, HolonomyElement(face_list = []),vertices[::-1],edges[::-1],paired_vertices[::-1],paired_edges[::-1],[x*(-1) for x in signs], index = i)
				...
				face = Face(None,HolonomyElement([i]),vertices,edges,paired_vertices,paired_edges, edge_orientations = signs,index = i)
			...
			if i % 2 == 0:
				orbit = FaceOrbit(face,[vertex.orbit for vertex in vertices],[edge.orbit for edge in edges],signs, int(i/2))
				orbit.add(face)
...
#-----------------------GRAPH THEORY-----------------------------------------
	...
	def _setup_graphs(self):
		edges = [(edge.tail.index, edge.head.index, edge.index) for edge in self.edges]
		edge_orbits = [(edge.tail.index, edge.head.index, edge.index) for edge in self.edge_orbits]
		self.digraph = DiGraph([[vertex.index for vertex in self.vertices],edges],loops = True, multiedges = True)
		edges = [(edge.tail, edge.head, edge) for edge in self.edge_orbits]
...
#----------------------Fundamental Group-----------------------------
	...
	def dual_group_ring(self):
		if self.sage_dual_group_ring == None:
...
#-------------------------------Twisted (and Dual) Homology---------------------------
	...
	def fox_deriv(self, holonomy, num, phi):
		if len(holonomy.holonomy) == 1:
			...
			return self.fox_deriv(holonomy[0], num, phi)+self.fox_deriv(holonomy[1:],num,phi)*phi(holonomy[0].inverse())
	...
	def dualB3(self, **args):
		...
		if dimension == 1:
			...
			#IS TAKING THE TRANSPOSE OKAY??
			return matrix.block(ring,domain_dimension,codomain_dimension, [a .transpose() for c in b for a in c], subdivide = dimension != 1).transpose()
	...
	def lift(self,vertex, head = None, tail = None):
		...
		if head == None and tail == None:
			...
			if vertex.orbit == self.head and vertex.orbit == self.tail:
				print('Warning: Given edge is a loop and head or tail not specified. Lift will not be unique, tail is chosen')
				tail = True
...
class Face(Cell,AbstractFace):
	def __init__(self,orbit = None, holonomy = None, vertices = None, edges = None, paired_vertices = None, paired_edges = None, edge_orientations = None, index = None, paired_face = None):
		Cell.__init__(self,orbit, holonomy, index)
		AbstractFace.__init__(self,vertices,edges, edge_orientations)
		self.paired_vertics = paired_vertices
...
class HolonomyElement(object):
	...
	#retuns false if it could not be reduced, true otherwise
	def reduce(self):
	...
	def apply_on_face(self, face):
		...
		return Face(face.orbit,self.compose(face.holonomy),new_vertices,new_edges,new_vertex_images,new_edge_images)
...
def test_fundamental_group(D):
	...
	for i in range(10):
		...
		print(M.homology())
		assert torsion_poly.sage_fundamental_group(D,False).abelian_invariants() == DD.dual_fundamental_group().abelian_invariants()
...
def save_graphs(D, string):
	DD = TwistableDomain(D)
	DD.orbit_digraph.plot(edge_labels = True).save(string + '_orbit_digraph.png')
	DD.orbit_graph.plot(edge_labels = True).save(string + '_orbit_graph.png')
	DD.digraph.plot(edge_labels = True).save(string + '_digraph.png')
	DD.graph.plot(edge_labels = True).save(string + '_graph.png')
...
def test_twisted_boundaries(D):
	...
	print('reduced d2*d3'+ '\n'*4)
	print((DD.reduced_dualB2(phi = phi,ring = RR, dimension = 4)*DD.reduced_dualB3(phi = phi,ring = RR, dimension = 4)).n(digits = 3))
	print('d2\n\n\n')
...
def test_noncommatative_group_ring_genus2():
	G = FreeGroup(4,'abcd')
	A = G.algebra(ZZ)
	def phi(hol):
...
def test_noncommatative_group_ring(D):
	DD = TwistableDomain(D)
